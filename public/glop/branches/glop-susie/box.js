var Box = Class.create({
  ...
  shape: function() {
    canvas.save()
	//var colorNow = "rgba("+this.colorglop[0]+","+this.colorglop[1]+","+this.colorglop[2]+","+this.alpha+")"
	...
	rect(this.w/-2,this.h/-2,this.w,this.h)
		// oldies
		fillStyle("rgba("+this.generation/12+",0,0,0.1")
		rect(this.w/-2,this.h/-2,this.w,this.h)
	canvas.restore()
...
//setup documentation 
  ...
  waterpush: function(push) { // make motion more smooth, too jerky
  	...
  	if (50 < this.distancefrom && this.distancefrom < 150) {
  		this.speed = 1
		trace(this.obj_id+" "+this.x+"***********"+this.y)
		if (this.xdifference == 0 || this.ydifference == 0 ){
			this.x += -push*this.speed*this.xdir/(this.xdifference+1)
  			this.y += -push*this.speed*this.ydir/(this.ydifference+1)
		} else {
			this.x += -push*this.speed*this.xdir/(this.xdifference)
  			this.y += -push*this.speed*this.ydir/(this.ydifference)
		}
  	} else {
  		this.speed = 0  		
  	}
  ...
  wavemotion: function() {
	...
	if (!collision){
		...
		this.y+=this.ydir*this.amplitude*Math.sin(this.x/30)
	}
	else if (collision){
		this.xdir *= -1
		this.speed = 4
  ...
  scared_: function() {
	...
	objects.each(function(object) {
		if (object.overlaps(me.x,me.y,50) && object.obj_id != me.obj_id){
			me.x += 0.1*(me.x - object.x)
			me.y += 0.1*(me.y - object.y)
			me.wiggle()
			
				//object.x += 0.05*(me.x - (object.x-object.w/2))
				//object.y += 0.05*(me.y - (object.y-object.h/2))
		}
	})
  },
  wiggle: function() {
...
/*
var cham = deep_clone(box)
cham.color = "red"
cham.obj_id = highest_id()+1
cham.fall = function() {
	
	me = this
	objects.each(function(object) {
		if (object.overlaps(me.x,me.y,100) && object.obj_id != me.obj_id){
			me.color = object.color
			me.water = object.water
		}
	})
}
...
var emb = deep_clone(box)
emb.color = "green"
emb.obj_id = highest_id()+1
emb.fall = function() {
	me = this
	objects.each(function(object) {
		if (object.overlaps(me.x,me.y,100) && object.obj_id != me.obj_id){
...
var shake = deep_clone(box)
shake.color = "yellow"
shake.obj_id = highest_id()+1
shake.fall = function() {
	me = this
	objects.each(function(object) {
		if (object.overlaps(me.x,me.y,100) && object.obj_id != me.obj_id){
...
shake.x = 100
shake.y = height/2+100
objects.push(shake)


var smaller = deep_clone(box)
smaller.color = "blue"
smaller.obj_id = highest_id()+1
smaller.fall = function() {
	me = this
	objects.each(function(object) {
		if (object.overlaps(me.x,me.y,100) && object.obj_id != me.obj_id && object.w >10 ){
			object.w -= 1
			object.h -= 1
		}
	})
...
smaller.x = width+100
smaller.y = height/2-200
objects.push(smaller)



var return2 = deep_clone(box)
return2.color = "black"
return2.obj_id = highest_id()+1
return2.fall = function() {
	me = this
	objects.each(function(object) {
		if (object.overlaps(me.x,me.y,100) && object.obj_id != me.obj_id ){
			if (object.w > 50){
...
return2.x = width+100
return2.y = height/2-50
objects.push(return2)



var magnet = deep_clone(box)
magnet.color = "gray"
magnet.obj_id = highest_id()+1
magnet.fall = function() {
	me = this
	objects.each(function(object) {
		if (object.overlaps(me.x,me.y,70) && object.obj_id != me.obj_id){
			if (!object.collision()){
				object.x += 0.05*(me.x - (object.x-object.w/2))
				object.y += 0.05*(me.y - (object.y-object.h/2))
			}
...
magnet.x = width+100
magnet.y = height/2+100
objects.push(magnet)


var virus = deep_clone(box)
virus.color = "purple"
virus.obj_id = highest_id()+1
virus.fall = function() {
	me = this
	objects.each(function(object) {
		if (object.overlaps(me.x,me.y,50) && object.obj_id != me.obj_id){
