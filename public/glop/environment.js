...
var Glop = {
	...
	init: function() {
		...
		if (!Config.resizable) Glop.set_size()
	},
	set_size: function(clear,custom_size) {
		clear = clear || false
		if (clear) $C.clear()
		if (!custom_size) { // see Canvas.to_print_data_url()
			if (Config.width) Glop.width = Config.width
			else Glop.width = document.viewport.getWidth()
			if (Config.height) Glop.height = Config.height
			else Glop.height = document.viewport.getHeight()
		}
	...
	draw: function(custom_size, force_draw) {
		...
		if (!draw_event.no_draw) {
			Glop.objects.each(function(object) {
				object.draw()
			})
		}
	...
	draw_powersave: function() {
		if (this.tail > 0 || Config.powersave == false) {
			if (this.tail > 0) this.tail -= 1
			Glop.draw()
...
document.observe('environment:init', Glop.init.bindAsEventListener(Glop))
...
document.observe('environment:init', Events.init)
...
document.observe('environment:init', $C.init.bindAsEventListener($C))
